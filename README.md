...
![工單清單與現場報工介面](docs/screenshots/02-work-orders.png)
...
打開 http://localhost:8080/ ，用 `operator01` 登入，點左邊任一張工單，然後照下面三步走。
下面的截圖都是實際跑出來的畫面。

**① 按「送出報工」**——綠色訊息顯示累計產出，下方列出這次扣掉哪些料、扣完各剩多少，
左側清單同步變成「生產中 10 / 60」。

![報工成功，並列出扣料明細](docs/screenshots/03-report-success.png)

**② 按「用同一把鍵再送一次」**——訊息變成黃色的「已受理過，未重複計算」。
注意冪等鍵與上一張完全相同，而左側進度**仍然是 10 / 60 沒有翻倍**，
扣料明細也消失了（因為這次沒有扣任何料）。這就是冪等鍵在做的事。

![重送同一個冪等鍵，未重複計算](docs/screenshots/04-idempotent-duplicate.png)

**③ 把良品數改成 `45` 再送**——紅色訊息指出缺哪一項、缺多少。
左側進度依然停在 10 / 60，代表這筆交易整個回滾了，
沒有留下「報工紀錄寫了、料卻沒扣」的半套資料。

![庫存不足，整筆交易回滾](docs/screenshots/05-insufficient-inventory.png)
...
POST /api/v1/production-reports
   ...
   ├─ 4. saveAndFlush 寫入報工紀錄 → UNIQUE 約束在此刻擋下併發重送
   └─ 5. 依 BOM 展開扣料並寫入物料流水，庫存不足 → 拋例外
   ↓
交易提交；任何一步失敗 → 全部回滾，不留半筆髒資料
```

第 4 步排在扣料**之前**是刻意的：先讓資料庫的唯一鍵擋掉重送，再去動庫存。
反過來寫也是對的（交易會回滾），但會白扣一輪料再回滾，尖峰時多消耗鎖與 IO。
